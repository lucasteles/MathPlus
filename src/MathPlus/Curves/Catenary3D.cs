...
public struct Catenary3D
{
    ...
    public float Length
    {
        get => cat2D.Length;
        set => cat2D.Length =
            value; // does not change evaluability of this type, since space hasn't changed
    }
    ...
    {
        cat2D = new CatenaryToPoint((p1 - p0).ToXY(), length);
    ...
    void ReadyForEvaluation()
    {
        ...
        space.RotateAroundYToInclude(P1, out var p1Local);
        cat2D.P = p1Local;
