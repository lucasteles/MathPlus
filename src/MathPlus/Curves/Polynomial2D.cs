...
namespace MathPlus {
	...
	public struct Polynomial2D : IPolynomialCubic<Polynomial2D, Vector2>, IParamCurve3Diff<Vector2> {
		...
		public int Degree => MathPlus.Max( x.Degree, y.Degree );
		...
		public Vector2 ProjectPoint( Vector2 point, out float t, int initialSubdivisions = 16, int refinementIterations = 4 ) {
			...
			for( int i = 1; i < initialSubdivisions; i++ ) {
				...
				if( MathPlus.SignAsInt( smp.distDeltaSq ) != MathPlus.SignAsInt( prevSmp.distDeltaSq ) ) {
		...
		private ResultsMax3<float> Intersect( Vector2 origin, Vector2 direction, bool rangeLimited = false, float minRayT = float.NaN, float maxRayT = float.NaN ) {
			...
			rel.C0 -= origin;
			float y0 = MathPlus.Determinant( rel.C0, direction ); // transform polynomial into the line space y components
			float y1 = MathPlus.Determinant( rel.C1, direction );
			float y2 = MathPlus.Determinant( rel.C2, direction );
			float y3 = MathPlus.Determinant( rel.C3, direction );
