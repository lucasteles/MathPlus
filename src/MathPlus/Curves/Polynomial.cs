...
namespace MathPlus {
	...
	[Serializable] public struct Polynomial : IPolynomialCubic<Polynomial, float> {
		...
		static bool ValueAlmost0( float v ) => MathPlus.Approximately( v, 0 );
		...
		static ResultsMax3<float> SolveDepressedCubicRoots( float p, float q ) {
			if( ValueAlmost0( p ) ) // triple root - one solution. solve x³+q = 0 => x = cr(-q)
				return new ResultsMax3<float>( MathPlus.Cbrt( -q ) );
			...
			if( discriminant < 0.00001 ) { // two or three roots guaranteed, use trig solution
				...
				float GetRoot( int k ) => pre * MathF.Cos( ( 1f / 3f ) * MathPlus.Acos( acosInner.ClampNeg1to1() ) - ( MathPlus.TAU / 3f ) * k );
			...
			if( discriminant > 0 && p < 0 ) { // one root
				float coshInner = ( 1f / 3f ) * MathPlus.Acosh( ( -3 * q.Abs() / ( 2 * p ) ) * MathF.Sqrt( -3 / p ) );
				float r = -2 * MathPlus.Sign( q ) * MathF.Sqrt( -p / 3 ) * MathPlus.Cosh( coshInner );
				return new ResultsMax3<float>( r );
			}

			if( p > 0 ) { // one root
				float sinhInner = ( 1f / 3f ) * MathPlus.Asinh( ( ( 3 * q ) / ( 2 * p ) ) * MathF.Sqrt( 3 / p ) );
				float r = ( -2 * MathF.Sqrt( p / 3 ) ) * MathPlus.Sinh( sinhInner );
				return new ResultsMax3<float>( r );
			}
