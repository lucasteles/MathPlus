...
[Serializable] public struct NUCatRomCubic3D : IParamSplineSegment<Polynomial3D, Vector3Matrix4x1> {
	...
	public float GetPointWeightAtKnotValue( int i, float u ) {
		var a = Mathf.InverseLerp( K0, K1, u );
		var b = Mathf.InverseLerp( K1, K2, u );
		var c = Mathf.InverseLerp( K2, K3, u );
		var d = Mathf.InverseLerp( K0, K2, u );
		var g = Mathf.InverseLerp( K1, K3, u );
		switch( i ) {
