...
namespace MathPlus {
	...
	[Serializable] public struct NUCatRomCubic3D : IParamSplineSegment<Polynomial3D, Vector3Matrix4x1> {
		...
		public float GetPointWeightAtKnotValue( int i, float u ) {
			float a = MathPlus.InverseLerp( K0, K1, u );
			float b = MathPlus.InverseLerp( K1, K2, u );
			float c = MathPlus.InverseLerp( K2, K3, u );
			float d = MathPlus.InverseLerp( K0, K2, u );
			float g = MathPlus.InverseLerp( K1, K3, u );
			switch( i ) {
