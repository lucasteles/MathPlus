...
namespace MathPlus {
	...
	public class CatRom2DSpline {
		...
		public float GetKnot( int index ) {
			ReadyKnotsAndCoefficients();
			if( index == -1 ) // extrapolate at the ends
				return MathPlus.Lerp( nodes[1].knot, nodes[0].knot, 2 );
			if( index == ControlPointCount )
				return MathPlus.Lerp( nodes[ControlPointCount - 2].knot, nodes[ControlPointCount - 1].knot, 2 );
		...
		public float GetKnotValue( float t ) => MathPlus.LerpClamped( KnotStart, KnotEnd, t );
		...
		float RangeCheckAndGetU( int curve, float t ) {
			...
			ReadyKnotsAndCoefficients();
			return MathPlus.Lerp( nodes[curve].knot, nodes[curve + 1].knot, t );
		}
