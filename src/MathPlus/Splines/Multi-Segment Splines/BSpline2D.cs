...
namespace MathPlus {
	...
	public class BSpline2D {
		...
		public float GetKnotValueAt( float t ) => MathPlus.Lerp( knots[degree], knots[knots.Length - degree - 1], t );
		...
		public Vector2 GetSegmentPoint( int segment, float t ) {
			...
			float u = MathPlus.Lerp( knotMin, knotMax, t );
		...
		public Vector2 Eval( int k, float u ) {
			...
			for( int r = 1; r < degree + 1; r++ ) {
				for( int j = degree; j > r - 1; j-- ) {
					float alpha = MathPlus.InverseLerpSafe( knots[j + k - degree], knots[j + 1 + k - r], u );
		...
		public Vector2 GetPointKnotAndWeightByLocalSpan( int point, float t ) {
			...
			float u = MathPlus.Lerp( knotStart, knotEnd, t );
