...
public class BSpline2D {
	...
	public BSpline2D( Vector2[] points, float[] knots, int degree = 3 ) {
		...
		var expectedKnotCount = SplineUtils.BSplineKnotCount( this.points.Length, this.degree );
	...
	public float GetKnotValueAt( float t ) => Mathf.Lerp( knots[degree], knots[knots.Length - degree - 1], t );
	...
	public BSpline2D Differentiate() {
		var dKnots = new float[KnotCount];
	...
	public Vector2 GetSegmentPoint( int segment, float t ) {
		...
		var u = Mathf.Lerp( knotMin, knotMax, t );
	...
	public Vector2 GetPointByKnotValue( float u ) {
		...
		if( u >= knots[InternalKnotIndexEnd] ) {
			...
			for( var j = 0; j < knots.Length; j++ ) { // find relevant interval
	...
	public Vector2 Eval( int k, float u ) {
		...
		for( var r = 1; r < degree + 1; r++ ) {
			for( var j = degree; j > r - 1; j-- ) {
				var alpha = Mathf.InverseLerpSafe( knots[j + k - degree], knots[j + 1 + k - r], u );
	...
	public Vector2 GetPointKnotAndWeightByLocalSpan( int point, float t ) {
		...
		var u = Mathf.Lerp( knotStart, knotEnd, t );
	...
	public Vector2 GetPointCollect( float t, ICollection<(Vector2 a, Vector2 b)> lines ) {
		...
		float tKnot = MathPlus.Lerp( knots[degree], knots[knots.Length - degree - 1], t );
