...
public static class ExtILinear2D
{
    ...
    [MethodImpl(INLINE)]
    public static Vector2 GetPoint<T>(this T linear, float t) where T : ILinear2D =>
        linear.Origin + linear.Dir * t;
    ...
    [MethodImpl(INLINE)]
    public static float ProjectPointTValue<T>(this T linear, Vector2 point)
        where T : ILinear2D =>
        linear.ClampTValue(Line2D.ProjectPointToLineTValue(linear.Origin, linear.Dir, point));

    /// <summary>Projects a point onto this line</summary>
    /// <param name="linear">The linear object to project onto (Ray2D, Line2D or LineSegment2D)</param>
    /// <param name="point">The point to project</param>
    [MethodImpl(INLINE)]
    public static Vector2 ProjectPoint<T>(this T linear, Vector2 point) where T : ILinear2D =>
        linear.GetPoint(linear.ProjectPointTValue(point));
    ...
    [MethodImpl(INLINE)]
    public static float Distance<T>(this T linear, Vector2 point) where T : ILinear2D =>
        MathF.Sqrt(DistanceSqr(linear, point));

    /// <summary>The shortest squared distance from this line to a point</summary>
    /// <param name="linear">The linear object to check distance from (Ray2D, Line2D or LineSegment2D)</param>
    /// <param name="point">The point to check the distance to</param>
    [MethodImpl(INLINE)]
    public static float DistanceSqr<T>(this T linear, Vector2 point) where T : ILinear2D =>
        (point - linear.ProjectPoint(point)).LengthSquared();
    ...
    [MethodImpl(INLINE)]
    public static bool Intersects<T, U>(this T linear, U other)
        where T : ILinear2D where U : ILinear2D => IntersectionTest.Linear(linear, other);
    ...
    [MethodImpl(INLINE)]
    public static bool Intersects<T>(this T linear, Circle2D circle) where T : ILinear2D =>
        IntersectionTest.LinearCircleIntersects(linear, circle);
    ...
    [MethodImpl(INLINE)]
    public static bool Intersect<T, U>(this T linear, U other, out Vector2 intersectionPoint)
        where T : ILinear2D where U : ILinear2D =>
    ...
    /// <param name="circle">The circle to test intersection against</param>
    [MethodImpl(INLINE)]
    public static ResultsMax2<Vector2> Intersect<T>(this T linear, Circle2D circle)
        where T : ILinear2D => IntersectionTest.LinearCircleIntersectionPoints(linear, circle);
