...
public struct Rotor3 {
	...
	public static Rotor3 FromToRotationDouble( Vector3 a, Vector3 b ) => new Rotor3( Vector3.Dot( a, b ), Mathf.Wedge( a, b ) );

	/// <summary>Creates a rotation from <c>a</c> to <c>b</c>. Note: Assumes both input vectors are normalized</summary>
	public static Rotor3 FromToRotation( Vector3 a, Vector3 b ) => new Rotor3( Vector3.Dot( a, b ) + 1, Mathf.Wedge( a, b ) ).Normalized();

	/// <summary>Constructs a unit rotor representing a rotation</summary>
	public Rotor3( float angle, Vector3 axis ) {
		var axisDual = new Bivector3( axis.X,axis.Y,axis.Z );
